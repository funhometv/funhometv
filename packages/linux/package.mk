...
case "$LINUX" in
  ...
  *)
    PKG_VERSION="5.4.228"
    PKG_SHA256="80253b2cac54c256dfdfd1061800aef907f88432090f7aef74c0ddf50f6e6524"
...
post_patch() {
  echo "we haoyq check------ post_patch is called"
  cp $PKG_KERNEL_CFG_FILE $PKG_BUILD/.config

  #we make it first exist , later generate haoyq 2023/1/5
  mkdir -p ${BUILD}/image
  touch ${BUILD}/image/initramfs.cpio
...
make_host() {
  echo "we haoyq check: make_host is called" 
...
makeinstall_host() {
  echo "we haoyq check: makeinstall_host is called"
  make \
    ARCH=${HEADERS_ARCH:-$TARGET_KERNEL_ARCH} \
    HOSTCC="$TOOLCHAIN/bin/host-gcc" \
    HOSTCXX="$TOOLCHAIN/bin/host-g++" \
...
pre_make_target() {
  echo " haoyq we ------ check pre_make_target is called"
  if [ "$TARGET_ARCH" = "x86_64" ]; then
...
make_target() {
  echo "we haoyq check ----------KERNEL_OVERLAY_DIR = $(get_kernel_overlay_dir)"
  kernel_make modules
  ...
  #we removed the lock haoyq @ 2023/1/6 for it make dead lock ; removed for a test. @2023/1/17
...
make_init() {
  echo " haoyq we --- make init -- is called"
 : # reuse make_target()
 #   echo "haoyq we check : ----------------:INSTALL is ${INSTALL}"
 #   ls -l ${INSTALL}
 #kernel_make
}

makeinstall_init() {
  echo " haoyq we --- makeinstall_init is called"
  if [ -n "$INITRAMFS_MODULES" ]; then
    ...
    for i in $INITRAMFS_MODULES; do
      ...
      echo "haoyq we check variable module: ----------- : ${module}"
      if [ -n "$module" ]; then
  ...
  # we haoyq removed at 2023/1/6 for linux build , we don't use UVESAFB
  #if [ "$UVESAFB_SUPPORT" = yes ]; then
  #  echo "haoyq we check ----------UVESAFB_SUPPORT: ${UVESAFB_SUPPORT}"
  #  echo "haoyq we check : ----------------:INSTALL is ${INSTALL}"
  #  mkdir -p ${INSTALL}
  #  ls -l ${INSTALL}
  #  echo "haoyq we check -------- get_full_module_dir:$(get_full_module_dir)"
  #  mkdir -p $INSTALL/usr/lib/modules
