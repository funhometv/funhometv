...
configure_target() {
  ...
  WAF_CMD_FORMAT=string PYTHON_CONFIG="$SYSROOT_PREFIX/usr/bin/python-config" \
  python_LDFLAGS="" python_LIBDIR="" \
  ./configure $PKG_CONFIGURE_OPTS
}

make_target() {
  #we haoyq change link redeclared  @ 2023/1/22 
  echo "we haoyq !!!!!!!!!!!!!!!!!!!!!!!! change the link name !!!!!!!!!!!!"
  sed -i "s|link|l_ink|g" $TOOLCHAIN/share/et/et_c.awk

  WAF_CMD_FORMAT=string  ./buildtools/bin/waf -v build --targets=$PKG_SAMBA_TARGET -j$CONCURRENCY_MAKE_LEVEL
}
