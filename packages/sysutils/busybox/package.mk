...
PKG_DEPENDS_INIT="toolchain libtirpc gnupg:init"
...
configure_host() {
  cd $PKG_BUILD/.$HOST_NAME
    cp -Pv $PKG_DIR/config/busybox-host.conf .config
...
makeinstall_host() {
  ...
  #we haoyq don't need a lot of modules of busybox in host @ 2023/1/7 , now 24 modules needed.
...
makeinstall_target() {
  ...
  #we haoyq changed to funhometv
  mkdir -p $INSTALL/usr/lib/funhometv
    #cp $PKG_DIR/scripts/functions $INSTALL/usr/lib/libreelec
    #we haoyq changed to funhometv
    cp $PKG_DIR/scripts/functions $INSTALL/usr/lib/funhometv
    cp $PKG_DIR/scripts/fs-resize $INSTALL/usr/lib/funhometv
    sed -e "s/@DISTRONAME@/$DISTRONAME/g" \
        -i $INSTALL/usr/lib/funhometv/fs-resize

    if listcontains "${FIRMWARE}" "rpi-eeprom"; then
      #cp $PKG_DIR/scripts/rpi-flash-firmware $INSTALL/usr/lib/libreelec
      cp $PKG_DIR/scripts/rpi-flash-firmware $INSTALL/usr/lib/funhometv
    fi

  mkdir -p $INSTALL/etc
    cp $PKG_DIR/config/profile $INSTALL/etc
    cp $PKG_DIR/config/inputrc $INSTALL/etc
   ######### we don't want busybox's httpd.conf overwrite apache2's httpd.conf
...
post_install() {
  ...
  #echo "haoyq we check: TOOLCHAIN ${TOOLCHAIN}"
  #echo "cryptpw is:"
  ls -l ${TOOLCHAIN}/bin/cryptpw
  #echo "ROOT_PASSWORD of image: ${ROOT_PASSWORD}" 
  #echo "check:"
  ...
  echo "show-version.service"
  enable_service show-version.service
  echo "var.mount"
  enable_service var.mount
  echo "var-log.mount"
  enable_service var-log.mount
  echo "fs-resize.service"
  enable_service fs-resize.service
  
  echo "after enable 5 service"
