...
PKG_URL="https://ftp.mozilla.org/pub/security/nss/releases/NSS_3_87_RTM/src/nss-3.87-with-nspr-4.35.tar.gz"
PKG_DEPENDS_HOST="nspr:host zlib:host"
#PKG_DEPENDS_HOST="zlib:host"
PKG_DEPENDS_TARGET="toolchain nss:host nspr zlib sqlite"
#PKG_DEPENDS_TARGET="toolchain nss:host zlib sqlite"
...
make_host() {
  ...
  echo "we haoyq check: INCLUDES=${INCLUDES} , CC=${CC} , CFLAGS=${CFLAGS}."
  #we haoyq disable -Werror for 3.87 as 2023/1/8  for certdb.c: In function 'CERT_UnlockCertRefCount': ../../../dist/public/nss/secport.h:121:24: error: implicit declaration of function 'PR_ASSERT_ARG'; did you mean 'PR_ASSERT'? [-Werror=implicit-function-declaration]
  echo "we haoyq check certdb.c ssl3con.c"
  ls -l lib/certdb/certdb.c
  sed -i "s'PORT_AssertArg'PR_ASSERT'g" lib/certdb/certdb.c
  ls -l lib/certdb/certdb.c
  ls -l lib/ssl/ssl3con.c
  sed -i "s'PORT_AssertArg'PR_ASSERT'g" lib/ssl/ssl3con.c
  ls -l lib/ssl/ssl3con.c
...
makeinstall_host() {
  ...
  #removed for error report in makeinstall. we haoyq @2023/1/9
