...
post_makeinstall_target() {
  ...
  # haoyq we changed the journald.conf size tune, use the bigger one  to get reasonnable log for debug. Old one is 10M or 12M max. @20210123
  sed -e "s,^.*RuntimeMaxUse=.*$,RuntimeMaxUse=512M,g" -i $INSTALL/etc/systemd/journald.conf
  sed -e "s,^.*RuntimeMaxFileSize=.*$,RuntimeMaxFileSize=512M,g" -i $INSTALL/etc/systemd/journald.conf
  sed -e "s,^.*SystemMaxUse=.*$,SystemMaxUse=1G,g" -i $INSTALL/etc/systemd/journald.conf
